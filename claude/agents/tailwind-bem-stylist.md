...
description: Use this agent when you need to style UI components using Tailwind CSS with BEM naming conventions. This includes tasks like creating new component styles, refactoring existing Tailwind utility clusters into maintainable BEM classes, organizing CSS layers, improving template readability, or ensuring consistent styling patterns across a codebase. Examples:\n\n<example>\nContext: User has written a new card component with inline Tailwind utilities.\nuser: "Create a product card component with an image, title, price, and add-to-cart button"\nassistant: "Here's the ProductCard component with the basic structure:"\n<component code with raw Tailwind utilities>\nassistant: "Now let me use the tailwind-bem-stylist agent to properly organize the styles using our BEM conventions and consolidate repeated utility patterns."\n<commentary>\nSince a new UI component was created with multiple styling concerns, use the tailwind-bem-stylist agent to consolidate utilities into BEM classes and ensure maintainable CSS organization.\n</commentary>\n</example>\n\n<example>\nContext: User wants to clean up a template with long, repetitive Tailwind class strings.\nuser: "This navbar has really messy classes, can you clean it up?"\nassistant: "I'll use the tailwind-bem-stylist agent to refactor these Tailwind utilities into organized BEM classes."\n<commentary>\nThe user is asking for CSS organization and cleanup, which is the core responsibility of the tailwind-bem-stylist agent.\n</commentary>\n</example>\n\n<example>\nContext: User just finished implementing a form with multiple input fields.\nuser: "Add a contact form with name, email, and message fields"\nassistant: "Here's the contact form implementation:"\n<form markup with functionality>\nassistant: "Let me now use the tailwind-bem-stylist agent to ensure the form styling follows our BEM conventions and consolidates any repeated input styles."\n<commentary>\nAfter creating UI with repeated elements (form inputs), proactively use the tailwind-bem-stylist agent to consolidate repeated styling patterns into maintainable BEM classes.\n</commentary>\n</example>
model: sonnet
color: yellow
---
