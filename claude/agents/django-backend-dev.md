...
description: Use this agent when you need to build, modify, or review Django backend code including endpoints, models, serializers, services, and admin configurations. This agent specializes in secure, database-agnostic Django development with proper access control, validation, and edge case handling.\n\nExamples:\n\n<example>\nContext: User needs to create a new API endpoint for managing user profiles.\nuser: "Create an endpoint that lets users update their own profile information including name and bio"\nassistant: "I'll use the django-backend-dev agent to build this endpoint with proper security and validation."\n<Task tool invocation to launch django-backend-dev agent>\n</example>\n\n<example>\nContext: User needs to add a new model with admin integration.\nuser: "Add a Subscription model that tracks user subscriptions with start date, end date, and plan type"\nassistant: "Let me use the django-backend-dev agent to create this model with proper constraints and admin configuration."\n<Task tool invocation to launch django-backend-dev agent>\n</example>\n\n<example>\nContext: User is building a multi-tenant feature that requires careful access control.\nuser: "I need an endpoint where organization admins can view all invoices for their organization"\nassistant: "This requires careful tenant-scoping and permission checks. I'll use the django-backend-dev agent to implement this securely."\n<Task tool invocation to launch django-backend-dev agent>\n</example>\n\n<example>\nContext: User just finished writing some Django code and needs it reviewed.\nuser: "Can you review the endpoint I just created?"\nassistant: "I'll use the django-backend-dev agent to review your code for security, permissions, and Django best practices."\n<Task tool invocation to launch django-backend-dev agent>\n</example>
model: sonnet
color: blue
---
